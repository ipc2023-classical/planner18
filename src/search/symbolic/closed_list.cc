...
void ClosedList::extract_path(const BDD &c, int h, bool fw,
    ...
    if (zeroCostClosed.count(h)) {
        ...
        while (steps0 < zeroCostClosed.at(h).size() && empty_intersection(cut, zeroCostClosed.at(h)[steps0])) {
            ...
            for (const auto & key : trs) { //TODO: maybe is best to use the inverse order
			  ...
			  cout << "Intersection: "; intersection.print(0,1););*/
                    if (!intersection.IsZero()) {
                        ...
                        steps0 = 0;
                        if (zeroCostClosed.count(h)) {
                            while (empty_intersection(cut,zeroCostClosed.at(h)[steps0])) {
...
std::pair<BDD, int> ClosedList::extract_partial_path(const map<int, vector<TransitionRelation>> &trs,
        BDD target, int h, bool fw,
        vector<OperatorID> &path) const {
    assert(closed.count(h));
    assert(!((closedTotal*target).IsZero()));
    assert(!((closed.at(h)*target).IsZero()));
    size_t steps0 = 0;
    if (zeroCostClosed.count(h)) {
        assert(trs.count(0));
        while (steps0 < zeroCostClosed.at(h).size() && empty_intersection(target, zeroCostClosed.at(h)[steps0])) {
            steps0++;
        }
        if (steps0 < zeroCostClosed.at(h).size()) {
            target *= zeroCostClosed.at(h)[steps0];
        } else {
            bool foundZeroCost = false;
            for (const TransitionRelation &tr : trs.at(0)) {
                if (foundZeroCost)
                ...
                for (size_t newSteps0 = 0; newSteps0 < steps0; newSteps0++) {
                    ...
                    if (!intersection.IsZero()) {
                        steps0 = newSteps0;
                        target = succ;
                        path.push_back((*tr.getOps().begin())->get_id());
                        foundZeroCost = true;
                        break;
                    }
                }
            ...
            for (const auto & key : trs) { //TODO: maybe is best to use the inverse order
                ...
                for (const TransitionRelation &tr : key.second) {
                    ...
                    if (!intersection.IsZero()) {
                        ...
                        steps0 = 0;
                        if (zeroCostClosed.count(h)) {
                            while (empty_intersection(target,zeroCostClosed.at(h)[steps0])) {
                                steps0++;
                                assert(steps0 < zeroCostClosed.at(newH).size());
                            }
                            target *= zeroCostClosed.at(h)[steps0];
...
ADD ClosedList::getHeuristicRealHNotClosed(int h_not_closed) const {
    BDD statesWithHNotClosed = !closedTotal;
    ADD h = mgr->mgr()->constant(0);
    //cout << "New heuristic with h [";
    for (auto &it : closed) {
        //cout << it.first << " ";
        int h_val = it.first;
        if (h_val != h_not_closed) {
            h += it.second.Add() * mgr->mgr()->constant(h_val);
        } else {
            statesWithHNotClosed += it.second;
        }
    }

    if (h_not_closed >= 0 && !statesWithHNotClosed.IsZero()) {
        h += statesWithHNotClosed.Add() * mgr->mgr()->constant(h_not_closed);
    }
